                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">Driver Zone
                                    </li>
                                </ol>
                            </div>
                        </div>
                                    ...
                                    <tbody id="searchContent">
                                        <?php 
                                            ...
                                            foreach($reports as $r) {
                                                if($r["job_branch"]=='ny') $branch='New York';
                                                if($r["job_branch"]=='nj') $branch='New Jersey';
                                                if($r["job_branch"]=='messny') $branch='Messenger NY';
                                                if($r["job_branch"]=='outside') $branch='Outside';
                                        ?>
                                        <tr>
                                            <td><?=$r["id"]?></td>
                                            <td><?=$r["driver_zone_name"]?></td>
                                            <td><?=$branch?></td>
                                            <td>
                                                <button class="btn-sm btn-primary" onclick="editInfo(<?= $r['id'] ?>)" data-toggle="modal" data-target="#editModal">Edit</button>
                                            </td>
                                        </tr>
                                        <?php } } ?>
                                    </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header py-2">
                    <h5 class="modal-title">Driver Zone Manage</h5>
                    <button type="button" class="close p-0 bg-transparent" style="right:20px;top:10px;box-shadow: none;" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                    <form action="<?= base_url() ?>/dispatch/driver_zone_update" method="post">
                    <div class="modal-body px-2">
                        <div class="form-group row m-0">
                            <label class="col-md-3">Zone No :</label>
                            <label class="col-md-9"><strong id="edit_no"></strong></label>
                            <input type="hidden" id="edit_zone_id" name="id">
                        </div>
                        <hr>
                        <div class="form-group row m-0">
                            <label class="col-md-3">Name :</label>
                            <input type="text" class="form-control col-md-9" id="edit_name" name="driver_zone_name">
                        </div>
                        <br>
                        <div class="form-group row m-0" id="branchSel">
                            <label class="col-md-3">Branch :</label>
                            <select class="form-control col-md-9" id="edit_branch" name="job_branch">
                                <option value="ny">New York</option>
                                <option value="nj">New Jersey</option>
                                <option value="messny">Messenger NY</option>
...
<?php require(APPPATH.'views\common\__footer.php'); ?>
    ...
    <script>
        ...
        function editInfo(id) {
                $.ajax({
                    type: "post",
                    url: "<?= base_url() ?>/dispatch/driver_zone_info",
                    data: "id=" +id,
                    dataType: "json",
                    success: function (response) {
                        $('#edit_no').html(response.id);
                        $('#edit_zone_id').val(response.id);
                        $('#edit_name').val(response.driver_zone_name);
                        $("#branchSel select").val(response.job_branch).change();
                    }
                });
            }
